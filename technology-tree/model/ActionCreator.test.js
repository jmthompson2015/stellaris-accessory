...
QUnit.test("setTechnology()", assert => {
  // Setup.
  const technologyKey = 4;

  // Run.
  const result = ActionCreator.setTechnology(technologyKey);

  // Verify.
  assert.ok(result);
  assert.equal(result.type, ActionType.SET_TECHNOLOGY);
  assert.equal(result.technologyKey, technologyKey);
});
