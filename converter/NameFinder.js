...
NameFinder.find = (buildingKey) =>
  new Promise((resolve) => {
    exec(
      `sh ../converter/nameDescription.sh ${buildingKey}`,
      (error, stdout) => {
        ...
        if (
          ...
          description.toLowerCase().endsWith(suffix1)
        ) {
          const newDesc = description.substring(
            1,
            description.length - suffix1.length
          );
